...
$(function () {
	...
	if (-1 == WebVideoCtrl.I_CheckPluginInstall()) {
		...
		/*window.open(ctxStatic +"/WebComponents.exe");*/
		return;
	}
	
	// 初始化插件参数及插入插件
	WebVideoCtrl.I_InitPlugin("100%", "720px", {
        iWndowType: 1,
        szColorProperty:"sub-border-select:00bb9c",
	...
	WebVideoCtrl.I_InsertOBJECTPlugin("divhik1");
	// 检查插件是否最新
...
function read(a){
	$.post("/a/aiSecurity/readf",{filepath:a},function(data){
		/*alert(data);*/
	});
}
// 格式化时间
...
function clickSetLocalCfg() {
	...
	arrXml.push("<BuffNumberType>2</BuffNumberType>");
	arrXml.push("<RecordPath>D:\\Web\\RecordFiles</RecordPath>");
	arrXml.push("<CapturePath>D:\\Web\\CaptureFiles</CapturePath>");
	arrXml.push("<PlaybackFilePath>D:\\Web\\PlaybackFiles</PlaybackFilePath>");
	arrXml.push("<PlaybackPicPath>D:\\Web\\PlaybackPics</PlaybackPicPath>");
	arrXml.push("<DownloadPath>D:\\Web\\DownloadFiles</DownloadPath>");
	arrXml.push("<IVSMode>0</IVSMode>");
...
// 登录
function clickLogin(a) {
			 var szIP = $("#divhik"+a).attr("divip"),
				szPort = $("#divhik"+a).attr("divport"),
				szUsername = $("#divhik"+a).attr("divname"),
				szPassword = $("#divhik"+a).attr("divpwd");
...
// 开始预览
function clickStartRealPlay(a) {
	var oWndInfo = WebVideoCtrl.I_GetWindowStatus(g_iWndIndex);
		szIP = $("#divhik"+a).attr("divip"),
