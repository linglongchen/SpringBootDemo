layui.define(['layer', 'contextMenu', 'form'], function (exports) {
    var contextMenu = layui.contextMenu;
    var layer = layui.layer;
    var form = layui.form;
    var cachedata = layui.layim.cache();   //复制layim数据
        ...
        register: function () {
            ...
            if (layim) {
                ...
                layim.on('ready', function (res) {
                    cachedata.mine.msgBox = 1;   //模拟有新消息的数据
                    if (cachedata.mine.msgBox != 0) {
                        //console.log('复制的所有数据',cachedata);
                ...
                layim.on('sendMessage', function (data) { //监听发送消息
                    ...
                    if (data.to.type == 'friend') {
                        console.log('朋友监听发送消息', data);data
                        im.sendMsg(data);
                    } else {
                        var _time = (new Date()).valueOf();//当前时间
    ...
    var im = {
        ...
        initListener: function (user, pwd) { //初始化监听
            ...
            conn.open({
                ...
                success: function (token) {
                    console.log('conn.open登陆成功', token)
                }
            });
            conn.listen({
                ...
                onPresence: function (message) {//监听对方的添加或者删除好友请求，并做相应的处理。
                    if (message.type == 'unsubscribe') {
                        ...
                        var avatar = 'http://localhost:8080/adssp/content/chat/layui/images/portrait/qq.jpg';
                        var options = {
        ...
        defineMessage: function (message, msgType) {
           ...
           $.ajaxSettings.async = false;
            $.post(ctx+'/hotelinfo/hotelInfo/hotelName', {"codeName":message.from}, function (result) {
                console.log(result);
                username = result.hotelInfo.hotelName;
                if (username == null || username == ""){
                    username ="邻奌通访客";
                }
                avatar = result.hotelInfo.hotelImg;
                if (avatar == null || avatar == ""){
                    avatar = "https://webappcommon.oss-cn-beijing.aliyuncs.com/common%2FdefultAvatar.png";
                }
            }, 'json');
            var data = {
                mine: false,
        ...
        sendMsg: function (data) {  //根据layim提供的data数据，进行解析
            ...
            console.log("我发送的内容"+data.mine.content);
            console.log("我发送的对象"+data.to.id);
            msg.set({
                msg: data.mine.content,
                to: data.to.id,         // 接收消息对象（必须是对方登录的账号名）
                roomType: false,
                ext: {  //ext方法扩展添加头像,在接消息时可以收到头像
                    // 'username': data.mine.username,
                    'avatar': data.mine.avatar
                },
                success: function (id, serverMsgId) {
                    console.log('发送文本成功', serverMsgId);
                    var sendData = {
                        to: data.to.id,
        ...
        userStatus: function (data) {
           /* if (data.userId) {
                $.get('http://yapi.demo.qunar.com/mock/11732/layim/onlineState', {userId: data.userId}, function (res) {
                    // var data = eval('(' + res + ')');
                    var data = res;
                    if (data.code == 0) {
                        if (data.data == 'online') {
