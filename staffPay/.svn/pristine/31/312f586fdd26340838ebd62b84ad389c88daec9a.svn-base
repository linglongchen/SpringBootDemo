jQuery(function() {
    function e(e) {
        ...
        e.on("statuschange", function(t, n) {
            ...
            s.remove()),
            "error" === t || "invalid" === t ? (console.log(e.statusText),f.hide(),
            d(e.statusText),
    ...
    var n, o = jQuery, r = o("#uploader"), l = o('<ul class="filelist"></ul>').appendTo(r.find(".queueList")), d = r.find(".statusBar"), p = d.find(".info"), c = r.find(".uploadBtn"), u = r.find(".placeholder"), f = d.find(".progress").hide(), m = 0, h = 0, g = window.devicePixelRatio || 1, v = 250 * g, b = 250 * g, k = "pedding", w = {}, x = function() {
    ...
    n = WebUploader.create({
        ...
        disableGlobalDnd: !0,
        chunked: !0,
        server: ctx + "/upload/uploadImg?width=" + $('#i_width').val() + "&height=" + $('#i_height').val(),
        fileNumLimit: 1,
   ...
   /* n.addButton({
    ...
    n.onUploadSuccess = function(file, result){
    	//alert(result._raw);
    	if(result._raw.indexOf('/') < 0){
    		var files = n.getFiles();
    		if(files.length > 0){
    			for(var i = 0; i < files.length; i++){
    				n.removeFile(files[i], true);
    			}
    		}
    		showMsgError(result._raw);
    		$('.filelist li').remove();
    		return void t("error");
    	}else{
    		$('#imgurl').val(result._raw);
    		if (f.hide(), c.text("开始上传").addClass("disabled"), a = n.getStats(), a.successNum && !a.uploadFailNum)
        		return void t("finish");
    	}
    },
   /* n.onUploadFinished = function(file, result){
    	//alert(321);
    	alert(result);
    	 if (f.hide(),
         c.text("开始上传").addClass("disabled"),
